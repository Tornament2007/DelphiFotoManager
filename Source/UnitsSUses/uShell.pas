...
// http://jqbook.narod.ru/delphi/winfilecopyclipbrd.htm
// Тольятти 2012
...
type
  ...
  WinExplorer = class
    ...
    class procedure FileToClipboard(const FilePath: string; MoveType: TWinMoveType = wCopy); overload;
    // выполнить операцию вставки в папку из буфера
    class procedure ClipboardPaste(const Target: string);
...
class procedure WinExplorer.ClipboardPaste(const Target: string);
{
    ...
    begin
      ...
      begin
        case Clipboard_SendType of
          5:
            Shell_DataOperations(Sourse, Target, FO_COPY, FOF_SIMPLEPROGRESS or
              FOF_RENAMEONCOLLISION);
          2:
            Shell_DataOperations(Sourse, Target, FO_MOVE, FOF_SIMPLEPROGRESS);
        end;
      end
      ...
      begin
        case Clipboard_SendType of
          5:
            Shell_DataOperations(Sourse, Target, FO_COPY, FOF_SIMPLEPROGRESS);
          2:
            Shell_DataOperations(Sourse, Target, FO_MOVE, FOF_SIMPLEPROGRESS);
        end;
      end;
...
class procedure WinExplorer.FileToClipboard(const FilePath: string;
  MoveType: TWinMoveType);
var
  s: TStringList;
begin
